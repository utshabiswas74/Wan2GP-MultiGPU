...
from ..model.model_misc import AttentionType, LayerScale, chunked_ffn_forward
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(self.fc1, nn.Linear) and isinstance(self.fc2, nn.Linear):
            x_list = [x]
            del x
            return chunked_ffn_forward(x_list, self.fc1.out_features, self.fc1.in_features, _forward)
        return _forward(x)
...
class Attention(nn.Module):
    ...
    def _apply_rope(self, q, k) -> Tuple[Tensor, Tensor]:
        ...
        return apply_rotary_enc(qk_list, freqs_cis=self.freqs_cis)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = shortcut.add_(x)
        x.add_(self.ls2(self.mlp(self.norm2(x))))
